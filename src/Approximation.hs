...
linearApproximation,
CalculatedPoints (Linear, xPoints, yPoints, Segment), Interval, Points
) where
...
calcLinearCoefficients xList xxList yList xyList =
    let
        lengthDouble = fromIntegral $ length xList
        sx = sum xList
        sxx = sum xxList
        sy = sum yList
        sxy = sum xyList
...
instance Eq CalculatedPoints where
  Linear xp1 yp1 == Linear  xp2 yp2 = xp1 == xp2 && yp1 == yp2
  Segment xp1 yp1 == Segment xp2 yp2 = xp1 == xp2 && yp1 == yp2
  Linear {} == Segment {} = False
